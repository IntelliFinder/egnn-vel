...
class TwoFDisInit(nn.Module):
    ...
    def forward(self,
        ...
        idx = torch.arange(N)
        tuple_pattern = torch.ones(size=(B, N, N), dtype=torch.int, device=ef_mixed.device)
        tuple_pattern[:, idx, idx] = 2
        tuple_pattern = self.pattern_embedding(tuple_pattern) # (B, N, N, k_tuple_dim)
        
        emb2 = ef_mixed * tuple_pattern 
        
        emb2 = nn.Identity(self.mix_lin(emb2) + emb2) #residual nn
...
class TwoFDisLayer(nn.Module):
    ...
    def forward(self, 
        ...
        kemb_out = nn.Identity(self.output_lin(self_message * kemb_multed) + self_message * kemb_multed)
